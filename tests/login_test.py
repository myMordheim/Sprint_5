...
class Test_Login_Page():
    ...
    def test_login_from_registration_page(self, driver):
        ...
        WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located((By.XPATH, registration_login_button)))
        driver.find_element(By.XPATH, registration_login_button).click()
    ...
    def test_login_from_forgotten_password_page(self, driver):
        driver.get(login_page_url)
        driver.find_element(By.LINK_TEXT, login_forgot_password).click()
        WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located((By.XPATH, login_link_on_forgot_password)))
        driver.find_element(By.XPATH, login_link_on_forgot_password).click()
        WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located((By.XPATH, login_enter)))
        driver.find_element(By.XPATH, login_email_input).send_keys(user_data[0])
        driver.find_element(By.XPATH, login_password_input).send_keys(user_data[1])
        driver.find_element(By.XPATH, login_button).click()
