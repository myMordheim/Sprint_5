...
class Test_transfer_into_personal_account():
    ...
    def test_transfer_from_PA_to_constructor(self):
        ...
        WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located((By.LINK_TEXT, PA_profile)))
        driver.find_element(By.LINK_TEXT, PA_to_order_page).click()
    ...
    def test_transfer_from_PA_to_constructor_by_click_on_logo(self):
        driver = webdriver.Chrome()
        driver.get(login_page_url)
        driver.find_element(By.XPATH, login_email_input).send_keys(user_data[0])
        driver.find_element(By.XPATH, login_password_input).send_keys(user_data[1])
